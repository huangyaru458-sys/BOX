...
class Stat {
  ...
  // 使用单例，只初始化一次
  static getInstance() {
    let space = uni_modules_uniStat_utssdk_common_utils_pageInfo.get_space(common_vendor.er.config);
    if (uni_modules_uniStat_utssdk_common_core_report.Report.uniCloudInstance == null) {
      if (space != null) {
        ...
        uni_modules_uniStat_utssdk_common_core_report.Report.uniCloudInstance = common_vendor.er.init(spaceData);
      } else {
        if (!Stat.no_space) {
